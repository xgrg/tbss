...
def randomise_parallel(in_file, out_basename, design_mat, mask, tcon,
                       num_perm=1000, demean=False, n_cpus=6,
                       sleep_interval=120, email_notify=None):
    ...
    contrasts = read(tcon)
    fp = [op.join(op.dirname(out_basename), '%s_part%s.con'
          % (op.basename(out_basename), str(s)))
    ...
    for i in range(0, min(n_cpus, len(contrasts))):
        rand = fsl.Randomise(in_file=in_file, mask=mask, tcon=fp[i],
                             design_mat=design_mat, demean=demean,
                             num_perm=num_perm, raw_stats_imgs=True,
                             tfce2D=True)
        ...
        s = '-o %s --skipTo=%s -V' % (out_basename, skipTo)
        cmd = rand.cmdline.replace('-o "randomise"', s)
    ...
    if email_notify is not None:
        ...
        for i, e in enumerate(commands):
            prefix = 'mailx -s "%s %s %s %s" < /dev/null "%s"'\
                    % (contrasts[i][0], tcon, e.split('&>')[-1], '%s',
                       email_notify)
